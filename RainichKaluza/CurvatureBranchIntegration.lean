...
/-- Coordinate components of a continuous covector. -/
def continuousCovectorCoordinates
    (v : CurvatureCoordinateSpace4 →L[ℝ] ℝ) : OneForm4 :=
  fun j => v (curvatureCoordinateDirection j)

/-- Coordinate extraction followed by reconstruction recovers the original
continuous covector. -/
theorem oneForm4ContinuousLinearMap_continuousCovectorCoordinates
    (v : CurvatureCoordinateSpace4 →L[ℝ] ℝ) :
    oneForm4ContinuousLinearMap (continuousCovectorCoordinates v) = v := by
  ext u
  change oneForm4Evaluate (continuousCovectorCoordinates v) u = v u
  conv_rhs => rw [← curvatureCoordinateDirection_expansion u]
  simp [oneForm4Evaluate, continuousCovectorCoordinates, mul_comm]

/-- Explicit coordinate jet of the normalized timelike fixed-probe
curvature eigen-one-form. -/
noncomputable def timelikeCurvatureEigenCovectorCoordinateJet
    (g : CurvatureCoordinateSpace4 →
      ContinuousBilinForm CurvatureCoordinateSpace4)
    (P : CurvatureCoordinateSpace4 → Matrix4)
    (probe : CurvatureCoordinateSpace4)
    (x : CurvatureCoordinateSpace4) : Fin 4 → OneForm4 :=
  fun k j => smoothTimelikeCurvatureEigenCovectorFDeriv g P probe x
    (curvatureCoordinateDirection k) (curvatureCoordinateDirection j)

/-- Explicit coordinate jet of the normalized spacelike fixed-probe
curvature eigen-one-form. -/
noncomputable def spacelikeCurvatureEigenCovectorCoordinateJet
    (g : CurvatureCoordinateSpace4 →
      ContinuousBilinForm CurvatureCoordinateSpace4)
    (P : CurvatureCoordinateSpace4 → Matrix4)
    (probe : CurvatureCoordinateSpace4)
    (x : CurvatureCoordinateSpace4) : Fin 4 → OneForm4 :=
  fun k j => smoothSpacelikeCurvatureEigenCovectorFDeriv g P probe x
    (curvatureCoordinateDirection k) (curvatureCoordinateDirection j)
...
theorem timelikeCurvatureEigenCovectorCoordinateJet_eq_fderiv
    (g : CurvatureCoordinateSpace4 →
      ContinuousBilinForm CurvatureCoordinateSpace4)
    (P : CurvatureCoordinateSpace4 → Matrix4)
    (probe x : CurvatureCoordinateSpace4)
    (hg : DifferentiableAt ℝ g x)
    (hP : DifferentiableAt ℝ P x)
    (ht : smoothMetricPairing g (smoothMatrixProjectedVector P probe)
      (smoothMatrixProjectedVector P probe) x < 0) :
    oneFormFieldCoordinateFDeriv
        (smoothTimelikeCurvatureEigenCovector g P probe) x =
      timelikeCurvatureEigenCovectorCoordinateJet g P probe x := by
  funext k j
  unfold oneFormFieldCoordinateFDeriv
  rw [smoothTimelikeCurvatureEigenCovector_fderiv_apply
    g P probe x (curvatureCoordinateDirection k) hg hP ht]
  rfl

/-- The displayed spacelike fixed-probe jet is the actual coordinate Frechet
derivative of the curvature eigen-one-form. -/
theorem spacelikeCurvatureEigenCovectorCoordinateJet_eq_fderiv
    (g : CurvatureCoordinateSpace4 →
      ContinuousBilinForm CurvatureCoordinateSpace4)
    (P : CurvatureCoordinateSpace4 → Matrix4)
    (probe x : CurvatureCoordinateSpace4)
    ...
    (hsign : 0 < smoothMetricPairing g (smoothMatrixProjectedVector P probe)
      (smoothMatrixProjectedVector P probe) x) :
    oneFormFieldCoordinateFDeriv
        (smoothSpacelikeCurvatureEigenCovector g P probe) x =
      spacelikeCurvatureEigenCovectorCoordinateJet g P probe x := by
  funext k j
  unfold oneFormFieldCoordinateFDeriv
...
/-- Build the constituent curvature patch directly from normalized fixed-probe
curvature eigen-one-forms.  Their displayed coordinate jets are generated by
the projector, normalization, and metric-dual product rules rather than
assumed as independent Frechet-derivative data. -/
noncomputable def ofFixedProbeCurvatureEigenCovectors
    (J : CurvatureCoordinateSpace4 → CurvatureScalarBranchJet4)
    (g : CurvatureCoordinateSpace4 →
      ContinuousBilinForm CurvatureCoordinateSpace4)
    (PA PB : CurvatureCoordinateSpace4 → Matrix4)
    ...
    (hbetaAmplitudeJet : ∀ x ∈ U,
      scalarFieldCoordinateFDeriv (fun y => (J y).y) x =
        (J x).betaAmplitudeJet)
    (hthetaAValue : ∀ x, (J x).thetaA = continuousCovectorCoordinates
      (smoothTimelikeCurvatureEigenCovector g PA probeA x))
    (hthetaBValue : ∀ x, (J x).thetaB = continuousCovectorCoordinates
      (smoothSpacelikeCurvatureEigenCovector g PB probeB x))
    (hdthetaA : ∀ x ∈ U, (J x).dthetaA =
      timelikeCurvatureEigenCovectorCoordinateJet g PA probeA x)
    (hdthetaB : ∀ x ∈ U, (J x).dthetaB =
      spacelikeCurvatureEigenCovectorCoordinateJet g PB probeB x) :
    CurvatureScalarBranchComponentPatch4 U := by
  have hPAOn : DifferentiableOn ℝ PA U := by
    change DifferentiableOn ℝ (fun x => fun i => fun j => PA x i j) U
    rw [differentiableOn_pi]
    intro i
    rw [differentiableOn_pi]
    intro j
    exact (hPA i j).differentiableOn_one
  have hPBOn : DifferentiableOn ℝ PB U := by
    change DifferentiableOn ℝ (fun x => fun i => fun j => PB x i j) U
    rw [differentiableOn_pi]
    intro i
    rw [differentiableOn_pi]
    intro j
    exact (hPB i j).differentiableOn_one
  have hfieldA :
      (fun x => oneForm4ContinuousLinearMap (J x).thetaA) =
        smoothTimelikeCurvatureEigenCovector g PA probeA := by
    funext x
    rw [hthetaAValue x,
      oneForm4ContinuousLinearMap_continuousCovectorCoordinates]
  have hfieldB :
      (fun x => oneForm4ContinuousLinearMap (J x).thetaB) =
        smoothSpacelikeCurvatureEigenCovector g PB probeB := by
    funext x
    rw [hthetaBValue x,
      oneForm4ContinuousLinearMap_continuousCovectorCoordinates]
  apply ofCoordinateFDerivs J halphaAmplitude hbetaAmplitude
  · rw [hfieldA]
    exact (contDiffOn_smoothTimelikeCurvatureEigenCovector
      probeA hg hPA htime).differentiableOn_one
  · rw [hfieldB]
    exact (contDiffOn_smoothSpacelikeCurvatureEigenCovector
      probeB hg hPB hspace).differentiableOn_one
  · exact halphaAmplitudeJet
  · exact hbetaAmplitudeJet
  · intro x hx
    rw [hfieldA]
    have hgAt := (hg.differentiableOn_one x hx).differentiableAt
      (hopen.mem_nhds hx)
    have hPAt := (hPAOn x hx).differentiableAt (hopen.mem_nhds hx)
    rw [timelikeCurvatureEigenCovectorCoordinateJet_eq_fderiv
      g PA probeA x hgAt hPAt (htime x hx)]
    exact (hdthetaA x hx).symm
  · intro x hx
    rw [hfieldB]
    have hgAt := (hg.differentiableOn_one x hx).differentiableAt
      (hopen.mem_nhds hx)
    have hPAt := (hPBOn x hx).differentiableAt (hopen.mem_nhds hx)
