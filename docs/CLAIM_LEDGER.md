...
| V3 | The helical black-string quotient supplies a nonnull, simple-spectrum, active Kaluza oracle satisfying exact EMD residuals, and its replacement point completes the selected fourth-order route with output \(3\). | **Exact symbolic**: `vt2-generic-helical-string`; `vt2-complete-detector-route` (21 checks) | The quadratic-tower calculation verifies scalar/residual/frame one-jets, all 128 raw channel components, \(A\), physical \(dA=d(\sqrt3 C)\), \(B\), the next-order residual, and output \(3\).  Literal quotient \(dA\) is identified by composing compiled physical-germ bridge theorems with exact helical Kaluza EMD patch/open gates; this benchmark-specific composition is neither a brute-force second-jet CAS expansion nor a Lean instance theorem. |
