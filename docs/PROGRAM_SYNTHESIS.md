...
| Scalar/Maxwell branch composition | every concrete relative-sign branch reaches an exact scalar rejection, Maxwell rejection, or Phase-III acceptance result with point witnesses retained | decision seam complete; the selected covector constructs the actual smooth self-adjoint scalar tensor/residual, while the generic canonical data prove and transport the reconstruction identity; identifying the actual fields with that transported pair and constructing the normal-gauge realizer remain |
| Maxwell two-form recovery and scalar/Maxwell field equations | pointwise transported seed, actual transported-seed derivatives, paired rescaled `C¹` realization, closed unweighted field, and closed positively weighted rotated Hodge flux complete; `S=R-V`, `q=√qSq`, smooth Maxwell projectors, verified pseudo-orthonormal/Lorentz frames, and a positive-cosine angle chart compose | prove the actual curvature-to-canonical identification, identify the selected complexion/Hodge channel with the metric residual, and reduce the compatible `C²` realizer equation by equation for unconditional sufficiency |
...
eigen-one-form equalities are proved by explicit Frechet product rules, and
the rational-diagonal/square-root calculation now proves the scalar-amplitude
pair. The four-identity constituent seam is therefore closed.
...
2. Formulate differential closure at the tensor level without prematurely
   ...
   **The four-way exact obstruction classifier, its zero/one/two local
   ...
   constructor are complete; all four concrete fixed-probe/amplitude
   coordinate-jet identities are now discharged.**
