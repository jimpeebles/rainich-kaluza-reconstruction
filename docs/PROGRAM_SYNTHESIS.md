...
| Scalar/Maxwell branch composition | every concrete relative-sign branch reaches an exact scalar rejection, Maxwell rejection, or Phase-III acceptance result with point witnesses retained | decision seam complete; actual smooth curvature-residual projectors now produce the closed physical field once the accepted residual/unit-complexion data are identified, while the normal-gauge realizer remains |
| Maxwell two-form recovery and scalar/Maxwell field equations | pointwise transported seed, actual transported-seed derivatives, rescaled `C¹` realization, and closed unweighted field complete; `S=R-V`, `q=√qSq`, smooth Maxwell projectors, verified pseudo-orthonormal/Lorentz frames, and a positive-cosine angle chart now compose | concrete identification of the accepted residual, self-adjoint orthonormal trivialization, and unit complexion pair plus the compatible `C²` realizer remain for unconditional sufficiency |
