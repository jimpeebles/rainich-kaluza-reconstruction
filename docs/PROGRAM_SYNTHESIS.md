...
| Maxwell two-form recovery and scalar/Maxwell field equations | pointwise transported seed, actual transported-seed derivatives, rescaled `C¹` realization, and closed unweighted field complete for actual `C²` frame/magnitude jets; compatible curvature-generated certificate and `C²` realizer open | required for unconditional sufficiency and for identifying the accepted curvature branch with the uplift input |
