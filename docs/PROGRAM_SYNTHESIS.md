...
| Scalar/Maxwell branch composition | every concrete relative-sign branch reaches an exact scalar rejection, Maxwell rejection, or Phase-III acceptance result with point witnesses retained | decision seam complete; actual smooth-frame data now produce the closed physical field, while concrete curvature instantiation and the normal-gauge realizer remain |
| Maxwell two-form recovery and scalar/Maxwell field equations | pointwise transported seed, actual transported-seed derivatives, rescaled `C¹` realization, and closed unweighted field complete; actual derivative arrays, angle-generated unit complexion data, and smooth fixed-probe coframe construction are automatic | concrete curvature selection/verification of the principal projectors, positive magnitude, and local angle plus the compatible `C²` realizer remain for unconditional sufficiency |
